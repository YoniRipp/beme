...
Status: **not started**. Review pass of 2026-09-14 re-verified the finding against the code
on `main` and widened it by one sibling bug. `shape.md` is accurate; two of its details are
sharpened below.
...
- [ ] Update `shape.md`'s fix block with the two corrections above once the fix lands
...
- [ ] Production arm accepts a non-empty string **or** a non-empty array of non-empty
      strings: `z.union([z.string().min(1), z.array(z.string().min(1)).min(1)])`
- [ ] Development arm gains the same array member alongside its existing string / boolean /
      undefined members
- [ ] Keep the custom production message (`CORS_ORIGIN must be set to an explicit origin in
      ...
      name.
- [ ] No change at either `cors()` call site. `@types/cors` declares
...
- [ ] `.filter(Boolean)` after the `.map()` in the split branch, so a trailing comma or a
      stray space cannot put an empty-string origin into the allowlist
- [ ] Without this, Task 2's production arm turns a trailing comma into a **new** boot
...
- [ ] `backend/src/controllers/auth.ts:95,99,102` — Twitter OAuth callback and error redirects
- [ ] `backend/src/routes/subscription.ts:21-23` — LemonSqueezy success / cancel URLs
- [ ] `backend/src/services/auth.ts:534` — password-reset base URL
- [ ] `backend/src/config/vapid.ts:11` — `mailto:admin@<origin>` VAPID subject

Fix:

- [ ] Derive `frontendOrigin` from the **first** parsed origin, not the raw string:
      explicit `FRONTEND_ORIGIN` wins, else `originList[0]`, else the existing fallback
- [ ] Mind the declaration order — `FRONTEND_ORIGIN` (`:115`) is read by the `CORS_ORIGIN`
      ...
      error.
- [ ] Preserve today's single-origin shape: one origin must still yield a **string**, not a
...
- [ ] Leave `:126-131` untouched
- [ ] Re-run both production negative cases after the change (tests below) to prove it
- [ ] Do **not** try to move these into the schema. They fire before `port` and `jwtSecret`
...
- [ ] Known, accepted gap: a list *containing* `'true'` (`CORS_ORIGIN=true,https://a.com`)
...
- [ ] The module validates at **import time** (`backend/CLAUDE.md` calls this out), so each
      case needs `vi.resetModules()`, `process.env` set, then `await import('./index.js')`.
      Restore `process.env` in `afterEach`.
- [ ] Set `PORT` in every case or `port` fails first and masks the assertion.
- [ ] `vi.mock('dotenv')` to a no-op. `:16-18` loads `backend/.env` and `.env.${NODE_ENV}`;
...
- [ ] Mock `../lib/logger.js` to keep the `JWT_SECRET` dev warning out of the output.

Development arm (`NODE_ENV` left at vitest's `test`):

- [ ] single origin → `corsOrigin === 'capacitor://localhost'` (string, **not** a 1-element array)
- [ ] comma list → `corsOrigin` deep-equals `['capacitor://localhost', 'http://localhost:5173']`
- [ ] comma list with spaces and trailing slashes → each element trimmed and slash-stripped
- [ ] comma list with empty segments (`'https://a.example.com, ,https://b.example.com,'`) →
      no empty strings in the result (Task 3)
- [ ] `CORS_ORIGIN=true` → `corsOrigin === 'true'`, the **string**. Today's parser does not
      coerce it; lock that in so the schema's boolean member isn't mistaken for this case.
- [ ] unset → `corsOrigin === true`, the **boolean** — this is what the boolean member is for
- [ ] comma list, `FRONTEND_ORIGIN` unset → `frontendOrigin === 'https://app.example.com'`,
      the first origin, not the joined string (Task 4)
- [ ] comma list, `FRONTEND_ORIGIN` set → the explicit value still wins

Production arm (`NODE_ENV=production`, `JWT_SECRET` set):

- [ ] single origin → boots, `corsOrigin` is the string
- [ ] comma list → **boots**, `corsOrigin` is the array. This is the regression test; it
      fails today with `corsOrigin: Expected string, received array`.
- [ ] `CORS_ORIGIN=true` → rejects with `CORS_ORIGIN must be an explicit origin in
      production, not true`
- [ ] `CORS_ORIGIN=` (empty string) → rejects with `CORS_ORIGIN must be explicitly set in
      production for security.`
- [ ] unset → rejects with the same `must be explicitly set` message
...
- [ ] mount `cors({ origin: ['https://a.example.com', 'https://b.example.com'], credentials: true })`
      and assert `access-control-allow-origin` echoes **each** configured origin
- [ ] assert a third origin gets **no** `access-control-allow-origin` header
- [ ] assert `access-control-allow-credentials: true` on the allowed ones — a wildcard would
...
- [ ] `backend/.env.example:26` — document the comma syntax next to `CORS_ORIGIN`, and say
      that `FRONTEND_ORIGIN` defaults to the first entry
- [ ] `docs/RUNNING-RAILWAY.md:33,42` and `docs/RUNNING-AWS.md:44` — the `CORS_ORIGIN` rows
...
- [ ] Leave `docker-compose.yml:28` and `backend/docker-compose.yml:29` on their single
      origin — no behaviour change needed there

## Verification

- [ ] `cd backend && npx tsc --noEmit` — clean
- [ ] `cd backend && npx vitest run` — existing suite still green, new config tests pass
- [ ] Boot the backend by hand for the eight rows in the verification table and confirm each
      now matches the intended column
- [ ] Rebase PR #295 on this and confirm its Capacitor allowlist boots
