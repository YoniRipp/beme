...
export async function findById(userId: string, id: string, client?: pg.Pool | pg.PoolClient): Promise<CycleEntry | null> {
  const db = client ?? getPool();
  const result = await db.query('SELECT ' + RETURNING + ' FROM cycle_entries WHERE id = $1 AND user_id = $2', [id, userId]);
...
export async function findByDate(userId: string, date: string, client?: pg.Pool | pg.PoolClient): Promise<CycleEntry | null> {
  const db = client ?? getPool();
  const result = await db.query('SELECT ' + RETURNING + ' FROM cycle_entries WHERE user_id = $1 AND date = $2::date', [userId, date]);
  return result.rows.length > 0 ? rowToEntry(result.rows[0]) : null;
}
