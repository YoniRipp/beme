/**
 * Trend and insight calculations for workouts, food entries and check-ins.
 *
 * This is the LIVE WEB CLIENT's implementation, moved here verbatim so both
 * clients compute insights identically. The mobile copy it replaces was
 * behaviourally equivalent function-for-function; the one genuine difference
 * between the two files was `CHART_COLORS`, which is platform-bound (the web
 * resolves `hsl(var(--chart-N))` through CSS custom properties, which React
 * Native cannot) and therefore deliberately stays in each client's own
 * `lib/analytics.ts`.
 */
...
import { getTrendPeriodBounds, WEEK_SUNDAY } from './dates';
