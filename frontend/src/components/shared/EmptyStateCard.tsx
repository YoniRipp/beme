...
export function EmptyStateCard({ onClick, icon: Icon = Plus, title, description }: EmptyStateCardProps) {
  return (
    <Card
      className="p-8 text-center cursor-pointer hover:border-primary/40 transition-colors press"
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      ...
      <p className="text-lg font-extrabold text-foreground tracking-tight">{title}</p>
